...
import com.ingenieraglobal.ecommerce.repositories.PresentacionProductoRepository;
    ...
    public CarritoDTO agregarProducto(Long usuarioId, AgregarAlCarritoRequest request) {
        ...
        if (request.getPresentacionId() != null) {
            PresentacionProducto presentacion = presentacionRepository
                    .findById(request.getPresentacionId())
                    .orElseThrow(() -> new RecursoNoEncontradoException("Presentacion no encontrada"));

            Optional<DetalleCarrito> detalle = detalleRepository
                    .findByCarritoIdAndPresentacionProductoId(carrito.getId(), request.getPresentacionId());
            ...
            if (detalle.isPresent()) {
                ...
                detalleRepository.save(new DetalleCarrito(carrito, presentacion, request.getCantidad()));
            }
            ...
            if (!producto.tieneStock()) {
                throw new RecursoNoEncontradoException("El producto no tiene stock disponible");
            }
            ...
            if (detalle.isPresent()) {
                ...
                detalleRepository.save(new DetalleCarrito(carrito, producto, request.getCantidad()));
            }
            ;
